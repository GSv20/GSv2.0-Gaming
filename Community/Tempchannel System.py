...
class JoinToCreate(commands.Cog):
    ...
    def init_db(self):
        async def create_voice_channels_table():
            async with aiosqlite.connect("Data/voice_channels.db") as db:
                await db.execute("""
        ...
        if after.channel and not before.channel and after.channel.id == 1216646094776438814:  # HIER DIE CHANNEL ID VOM VOICE
            async with aiosqlite.connect("Data/voice_channels.db") as db:
                ...
                if not channel_info:
                    ...
                    category = guild.get_channel(1203327799373594644)  # HIER DIE CATEGORIE ID
                    ...
                    await db.execute("INSERT INTO voice_channels (user_id, channel_id) VALUES (?, ?)",
                    ...
                    rechtevergeben_button = discord.ui.Button(style=discord.ButtonStyle.gray,
                                                              label="Eigentum-Übertragen", emoji="👑")
                    delete_button = discord.ui.Button(style=discord.ButtonStyle.grey,
                                                      label=f"{voice_channel.name} Löschen.", emoji="🧨")

                    async def call1(interaction):
                        channel_id = interaction.message.channel.id
                        async with aiosqlite.connect("Data/voice_channels.db") as db:
                            cursor = await db.execute("SELECT * FROM voice_channels WHERE channel_id = ?",
                                                      (channel_id,))
                            channel_info = await cursor.fetchone()

                            if not channel_info or interaction.user.id != channel_info[0]:
                                await interaction.response.send_message("Du bist nicht der Besitzer des Channels.",
                                                                        ephemeral=True)
                                return
                            else:
                                def check(message):
                            ...
                            await interaction.response.send_message(
                                "Bitte gib die gewünschte Benutzerlimitierung für deinen Kanal ein:", ephemeral=True)
                            try:
                                ...
                                if 0 < limit <= 99:
                                    ...
                                    await interaction.followup.send(f"Benutzerlimit des Kanals auf {limit} gesetzt.",
                                                                    ephemeral=True)
                                ...
                                await interaction.followup.send("Zeitlimit überschritten. Bitte versuche es erneut.",
                                                                ephemeral=True)
                            except ValueError:
                                await interaction.followup.send(
                                    "Ungültige Eingabe. Bitte gib eine Zahl zwischen 1 und 99 ein.", ephemeral=True)
                    ...
                    async def call2(interaction):
                        ...
                        async with aiosqlite.connect("Data/voice_channels.db") as db:
                            ...
                            if not channel_info or interaction.user.id != channel_info[0]:
                                ...
                                await interaction.response.send_message(
                                    "Bitte gib den neuen Namen für deinen Kanal ein:", ephemeral=True)
                                try:
                                    response = await interaction.client.wait_for('message', check=check, timeout=30)
                                    new_name = response.content
                                    voice_channel = interaction.message.channel
                                    await voice_channel.edit(name=new_name)
                                    await interaction.followup.send(f"Kanal erfolgreich in `{new_name}` umbenannt.",
                                                                    ephemeral=True)
                                    await asyncio.sleep(1)
                                    await response.delete()
                                except asyncio.TimeoutError:
                                    await interaction.followup.send(
                                        "Zeitlimit überschritten. Bitte versuche es erneut.", ephemeral=True)
                    ...
                    async def call5(interaction):
                        ...
                        async with aiosqlite.connect("Data/voice_channels.db") as db:
                            ...
                            if not channel_info or interaction.user.id != channel_info[0]:
                                ...
                                await interaction.response.send_message(
                                    "Bitte gib die ID des Benutzers an, den du bannen möchtest:", ephemeral=True)
                                try:
                    ...
                    async def call6(interaction):
                        ...
                        async with aiosqlite.connect("Data/voice_channels.db") as db:
                            ...
                            if not channel_info or interaction.user.id != channel_info[0]:
                                ...
                                await interaction.response.send_message(
                                    "Bitte gib die ID des Benutzers an, den du aus dem Kanal kicken möchtest:",
                                    ephemeral=True)
                                try:
                                    response = await self.bot.wait_for('message', timeout=30)
                                    user_id = int(response.content)
                    ...
                    async def call7(interaction):
                        ...
                        async with aiosqlite.connect("Data/voice_channels.db") as db:
                            ...
                            if not channel_info:
                                await interaction.response.send_message(
                                    "Der Voice Channel existiert nicht in der Datenbank.", ephemeral=True)
                                return

                            current_owner = interaction.guild.get_member(channel_info[0])
                            if current_owner is None:
                                await interaction.response.send_message(
                                    "Der aktuelle Besitzer ist nicht mehr auf dem Server.", ephemeral=True)
                                return
                            ...
                            if interaction.user.id == current_owner.id:
                                ...
                                try:
                                    ...
                                    if new_owner:
                                        await db.execute("UPDATE voice_channels SET user_id = ? WHERE channel_id = ?",
                                                         (new_owner.id, channel_id))
                                        await db.commit()
                                        await interaction.followup.send(
                                            f"{new_owner.mention} ist jetzt der neue Besitzer des Kanals.",
                                            ephemeral=True)
                                        await asyncio.sleep(1)
                                        await response.delete()
                                    else:
                                    ...
                                    await interaction.followup.send(
                                        "Ungültige Benutzer-ID. Bitte gib eine korrekte Benutzer-ID an.",
                                        ephemeral=True)
                            else:
                                if current_owner.voice is not None:
                                    await interaction.response.send_message(
                                        "Der aktuelle Besitzer ist online im Voice Channel. Nur er kann das Eigentum übertragen.",
                                        ephemeral=True)
                                    return

                                await db.execute("UPDATE voice_channels SET user_id = ? WHERE channel_id = ?",
                                                 (interaction.user.id, channel_id))
                                await db.commit()
                                await interaction.response.send_message(
                                    f"{interaction.user.mention} ist jetzt der neue Besitzer des Kanals.",
                                    ephemeral=True)
